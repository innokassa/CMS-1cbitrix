...
class ReceiptStorageConcrete implements ReceiptStorageInterface
{
    ...
    {
        $where = $this->where($filter);
        $sql = sprintf("SELECT MIN($column) FROM `%s` WHERE $where", self::$table);
    ...
    {
        $where = $this->where($filter);
        $sql = sprintf("SELECT MAX($column) FROM `%s` WHERE $where", self::$table);
        $result = $this->db->query($sql, true)[0];
        return current($result);
    }
    ...
    {
        $where = $this->where($filter);
        $sql = sprintf("SELECT COUNT(*) FROM `%s` WHERE $where", self::$table);
        $result = $this->db->query($sql, true)[0];
        return current($result);
    }
